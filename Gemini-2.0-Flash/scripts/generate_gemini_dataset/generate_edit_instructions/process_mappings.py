...
from api_call import get_edit_instructions_async  # unchanged API function
...
async def process_entry(entry: dict, limiter: AsyncRateLimiter, prompt_template: str, logger: Logger) -> str:
    output_old_path = Path(entry["output_explanation_path"])
    parts = list(output_old_path.parts)
    parts[-3] = "additional_edit_instructions"
    new_output_path = Path(*parts)

    out = new_output_path / entry["output_explanation_name"]
    if out.exists():
    ...
    try:
        ...
        async with limiter:
            explanation = await get_edit_instructions_async(real_img, prompt_name = prompt_template)
    except Exception as e:
